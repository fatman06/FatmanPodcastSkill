...
appToken = 'd1502400-3c2b-11a7-13ba-02f814b60257'
vi = VoiceInsights()
...
def build_card(title, content, ctype='Simple',image=''):
	...
	obj = None
	if ctype=='Standard':
		obj = {'card': {'type':ctype, 'title': title, 'text': content[:1800]}}
		if image !='':
			obj['card']['image'] = {"largeImageUrl":'https://s3.amazonaws.com/podbuddy-assets/{}.jpeg'.format(image)}
	else:
		obj = {'card': {'type':ctype, 'title': title, 'content': content[:1800]}}
	return obj
...
def on_intent(intent_request, session, context=None):
	...
	if intent_name == "AMAZON.HelpIntent":
		...
		response = handle_confirm_request(intent,session) 
		...
		try:
			...
			response = build_response(token["url"], None, intent_request["context"]["AudioPlayer"]["offsetInMilliseconds"],token) 
		except:
